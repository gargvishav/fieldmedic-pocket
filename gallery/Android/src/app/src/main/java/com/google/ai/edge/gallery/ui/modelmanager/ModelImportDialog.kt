...
private fun importModel(
  ...
  coroutineScope.launch(Dispatchers.IO) {
    ...
    // Fast-path: if the source URI resolves to a real file on the same FUSE volume
    // (typical for SAF picks from /sdcard/Download), rename instead of byte-copying.
    // Avoids needing an extra fileSize bytes free during a multi-GB model import.
    val sameVolumeSource: File? = run {
      if (uri.authority != "com.android.externalstorage.documents") return@run null
      val docId = uri.lastPathSegment ?: return@run null
      val parts = docId.split(":", limit = 2)
      if (parts.size != 2 || parts[0] != "primary") return@run null
      File("/storage/emulated/0/${parts[1]}")
    }
    if (sameVolumeSource != null && sameVolumeSource.isFile) {
      if (outputFile.exists()) outputFile.delete()
      if (sameVolumeSource.renameTo(outputFile)) {
        Log.d(TAG, "fast-path rename: ${sameVolumeSource.path} -> ${outputFile.path}")
        onProgress(1f)
        onDone()
        return@launch
      }
      Log.d(TAG, "fast-path rename failed; falling back to copy")
    }
