...
/**
 * Strips the FieldMedic language-mode directive from a user message for display.
 * The directive is still sent to the model — only the visible bubble is cleaned.
 * Handles Hindi, Bengali, Tamil, and Marathi prefixes (anything matching "[Respond in ...]").
 */
private fun cleanForDisplay(content: String): String {
  if (!content.startsWith("[Respond in ")) return content
  val end = content.indexOf(']')
  if (end == -1) return content
  return content.substring(end + 1).trimStart()
}
...
fun MessageBodyText(message: ChatMessageText, inProgress: Boolean) {
  SelectionContainer {
    if (message.side == ChatSide.USER) {
      MarkdownText(
        text = cleanForDisplay(message.content),
